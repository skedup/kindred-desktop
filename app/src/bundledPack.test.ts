...
import walkFrames from '../../visual-packs/kindred-default/motions/walk.json'
...
describe('bundled desktop visual pack', () => {
  it('resolves every manifest and frame source from packaged assets', () => {
    ...
    const frameSources = [rawFrames, eatFrames, settleFrames, sleepFrames, walkFrames].flatMap(
      (frames) => [...frames.enter, ...frames.loop],
    )
  ...
  it('ships action-specific body motion for bundled frame animations', () => {
    ...
    expect(manifest.motions[manifest.action_motions.walk]?.source).toBe('motions/walk.json')
  })

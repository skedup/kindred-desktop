#!/usr/bin/env python3
"""Promote the approved ``sleep-v2`` loop into the bundled visual pack."""
...
from tools.visual_pipeline.sleep_validate import require_exact_inventory, validate
from tools.visual_pipeline.webp_rgba import webp_rgba, write_lossless_webp

RUNTIME_DIRECTORY = Path("visual-packs/kindred-default/assets/body/sleep-v2")
LEGACY_RUNTIME_DIRECTORY = Path("visual-packs/kindred-default/assets/body/frame1/sleep")
MOTION_MANIFEST = Path("visual-packs/kindred-default/motions/sleep.json")
...
def motion_payload(*, frame_count: int = FRAME_COUNT) -> dict[str, object]:
    return {
        ...
        "enter": [],
        "loop": [
            f"assets/body/sleep-v2/sleep-{index:03d}{RUNTIME_FRAME_SUFFIX}"
...
def _runtime_paths(directory: Path, *, frame_count: int) -> list[Path]:
    expected = [
        directory / f"sleep-{index:03d}{RUNTIME_FRAME_SUFFIX}" for index in range(frame_count)
    ]
    actual = sorted(directory.iterdir()) if directory.exists() else []
    if actual != expected:
        raise SystemExit(
            f"sleep_runtime_inventory_invalid:expected={frame_count}:actual={len(actual)}"
        )
    return expected
...
def promote(repository: Path, *, frame_count: int = FRAME_COUNT) -> None:
    ...
    source_paths = require_exact_inventory(source_root / FRAME_DIRECTORY)
    if len(source_paths) != frame_count:
        raise SystemExit(
            f"sleep_source_inventory_invalid:expected={frame_count}:actual={len(source_paths)}"
        )
    ...
    with tempfile.TemporaryDirectory(
        prefix=".sleep-v2-stage-", dir=runtime_directory.parent
    ) as temporary:
        temporary_root = Path(temporary)
        staged_directory = temporary_root / "sleep-v2"
        staged_directory.mkdir()
        for index, source in enumerate(source_paths):
            pixels = source_rgba(source, size=RUNTIME_SIZE)
            destination = staged_directory / f"sleep-{index:03d}{RUNTIME_FRAME_SUFFIX}"
            write_lossless_webp(destination, size=RUNTIME_SIZE, pixels=pixels)
            if webp_rgba(destination, size=RUNTIME_SIZE) != pixels:
                raise SystemExit(f"sleep_staged_runtime_pixel_mismatch:{index}")

        _runtime_paths(staged_directory, frame_count=frame_count)
        staged_manifest = temporary_root / "sleep.json"
    ...
    for index, (source, runtime) in enumerate(zip(source_paths, runtime_paths, strict=True)):
        if source_rgba(source, size=RUNTIME_SIZE) != webp_rgba(runtime, size=RUNTIME_SIZE):
            raise SystemExit(f"sleep_promoted_runtime_pixel_mismatch:{index}")
    if json.loads(motion_manifest.read_text(encoding="utf-8")) != motion_payload(
        frame_count=frame_count
    ):
        raise SystemExit("sleep_promoted_runtime_schedule_invalid")

    if legacy_directory.exists():
        shutil.rmtree(legacy_directory)

    print(f"SLEEP_V2_PROMOTED frames={frame_count} fps={FPS} destination={RUNTIME_DIRECTORY}")
